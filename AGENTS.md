...
- **One products nav only:** `src/app/products/layout.tsx` renders `Sidebar` (top category sub-bar with module dropdowns). Category layouts (`structural`, `machine`, etc.) must be passthrough wrappers — never import `Sidebar` there.
- **No `DashboardLayout` on product pages:** module chrome comes from `CalculatorLayout` + the products category sub-bar / dropdowns. `npm run validate:layout` enforces this (also runs before `npm run build`).
...
- **CI benchmarks:** `npm run test:verification` — 81 JSON cases in `src/data/verification/` against `src/lib/qa/moduleSolverRegistry.ts` (67 solvers).
