...
- **One products nav only:** `src/app/products/layout.tsx` renders `ProductsCategoryBar` (category chip links on hub/landings; breadcrumb strip on module pages). Category layouts (`structural`, `machine`, etc.) must be passthrough wrappers — never import products nav there. Category browse lives at `/products/{categoryId}`.
- **No `DashboardLayout` on product pages:** module chrome comes from `CalculatorLayout` + the products category bar / breadcrumb. `npm run validate:layout` enforces this (also runs before `npm run build`).
