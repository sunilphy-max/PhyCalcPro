...
import {
  HOUSING_COPILOT_EXAMPLES,
  runHousingCopilotSession,
  type HousingCopilotApplyPayload,
  type HousingCopilotSession,
} from "@/lib/copilot/housingCopilot";

type Props = {
  onApply: (payload: HousingCopilotApplyPayload) => void;
};

export default function HousingCopilotPanel({ onApply }: Props) {
  const [text, setText] = useState("");
  const [session, setSession] = useState<HousingCopilotSession | null>(null);

  const run = (brief: string) => {
    const value = brief.trim();
    if (!value) return;
    setText(brief);
    setSession(runHousingCopilotSession(value));
  };
    ...
    <section className="overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50/90 via-white to-violet-50/50 shadow-sm dark:border-indigo-900/40 dark:from-indigo-950/40 dark:via-slate-900 dark:to-violet-950/30">
      <div className="flex items-start gap-3 border-b border-indigo-200/50 px-4 py-3 dark:border-indigo-900/40">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Housing copilot</h2>
          <p className="text-xs leading-relaxed text-slate-600 dark:text-slate-400">
            Describe bore, Fr/Fa, and speed — sizes bolt count and bolt circle for body SF. Mount style
            stays free unless named in the brief.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 p-4">
        <textarea
          id="housing-copilot-brief"
          value={text}
          onChange={(event) => setText(event.target.value)}
          rows={2}
          placeholder="e.g. Pillow block, 40 mm bore, 5 kN radial at 1500 rpm"
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap gap-1.5">
            {HOUSING_COPILOT_EXAMPLES.map((example) => (
              <button
                key={example}
                type="button"
                onClick={() => run(example)}
          ...
          {session.design?.best ? (
            ...
            <div className="flex items-start gap-2 rounded-xl border border-amber-200/80 bg-amber-50/60 p-3 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/25 dark:text-amber-100">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 opacity-90">Include bore (mm) and radial load. Example: &quot;40 mm bore, 5 kN radial&quot;.</p>
