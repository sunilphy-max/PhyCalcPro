...
export type GenericRiskLevel = "low" | "medium" | "high";

export type GenericDiagnosisFinding = {
  category: string;
...
type Props = {
  overallRisk: GenericRiskLevel;
  summary: string;
  findings: GenericDiagnosisFinding[];
  recommendations?: GenericRecommendation[];
};

export default function GenericDiagnosisPanel({
  overallRisk,
  summary,
  findings,
  recommendations = [],
          ...
          findings.map((finding, index) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-500">{finding.categoryLabel}</span>
            ...
            {recommendations.map((option) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300">{option.detail}</p>
                </div>
                {option.onApply ? (
                  <button
                    type="button"
