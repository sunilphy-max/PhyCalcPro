...
import {
  PLAIN_BEARING_COPILOT_EXAMPLES,
  runPlainBearingCopilotSession,
  type PlainBearingCopilotApplyPayload,
  type PlainBearingCopilotSession,
} from "@/lib/copilot/plainBearingCopilot";

type Props = {
  onApply: (payload: PlainBearingCopilotApplyPayload) => void;
};

export default function PlainBearingCopilotPanel({ onApply }: Props) {
  const [text, setText] = useState("");
  const [session, setSession] = useState<PlainBearingCopilotSession | null>(null);

  const run = (brief: string) => {
    const value = brief.trim();
    if (!value) return;
    setText(brief);
    setSession(runPlainBearingCopilotSession(value));
  };
    ...
    <section className="overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50/90 via-white to-violet-50/50 shadow-sm dark:border-indigo-900/40 dark:from-indigo-950/40 dark:via-slate-900 dark:to-violet-950/30">
      <div className="flex items-start gap-3 border-b border-indigo-200/50 px-4 py-3 dark:border-indigo-900/40">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Plain bearing copilot</h2>
          <p className="text-xs leading-relaxed text-slate-600 dark:text-slate-400">
            Describe load, speed, and shaft diameter — sizes journal geometry for film screening. Does not
            chain rolling bearings or housing.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 p-4">
        <textarea
          id="plain-bearing-copilot-brief"
          value={text}
          onChange={(event) => setText(event.target.value)}
          rows={2}
          placeholder="e.g. Journal bearing, 5 kN at 1200 rpm, 50 mm shaft"
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap gap-1.5">
            {PLAIN_BEARING_COPILOT_EXAMPLES.map((example) => (
              <button
                key={example}
                type="button"
                onClick={() => run(example)}
        ...
        <div className="space-y-3 border-t border-indigo-200/40 px-4 py-3 dark:border-indigo-900/30">
          {(tokenSummary.length > 0 || session.hints.length > 0) && (
            <div className="flex flex-wrap gap-1.5">
              {tokenSummary.map((token, index) => (
                ...
                >
                  {token.param}: {token.value} {token.unit}
                </span>
              ))}
          ...
          {session.design?.best ? (
            ...
            <div className="flex items-start gap-2 rounded-xl border border-amber-200/80 bg-amber-50/60 p-3 text-xs text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/25 dark:text-amber-100">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold">Could not size from this brief</p>
                <p className="mt-1 opacity-90">Include load (kN) and speed (rpm). Example: &quot;5 kN at 1200 rpm, 50 mm&quot;.</p>
                {session.canApplyLoads ? (
                  <button
                    type="button"
                    onClick={() => onApply(session.apply)}
                    className="mt-2 rounded-lg bg-amber-700 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-amber-600"
                  >
                    Apply loads to form
                  </button>
                ) : null}
