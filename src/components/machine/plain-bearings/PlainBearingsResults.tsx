        ...
        <>
          <CalculatorMetricGrid cols={2}>
            ...
            <CalculatorMetricCard
              ...
              tone="blue"
            />
            <CalculatorMetricCard label="Sommerfeld S" numericValue={result.sommerfeldNumber} unit="—" tone="blue" />
            {result.bearingType === "journal" ? (
              <CalculatorMetricCard label="Eccentricity ε" numericValue={result.eccentricityRatio} unit="—" tone="purple" />
            ) : null}
            {result.specificLoadPa != null ? (
              <CalculatorMetricCard
                label="Specific load"
                numericValue={result.specificLoadPa} unit="Pa"
                tone="orange"
              />
            ) : null}
            {result.unitLoad != null ? (
              <CalculatorMetricCard label="Unit load" numericValue={result.unitLoad} unit="Pa" tone="orange" />
            ) : null}
            <CalculatorMetricCard
              label="Minimum film thickness"
              numericValue={fromBase(result.minFilmThickness, "length", lengthUnit)} unit={lengthUnit}
              status={adequateFilm ? "safe" : "danger"}
            />
            <CalculatorMetricCard label="Power loss" numericValue={result.powerLoss} unit="W" tone="orange" />
            ...
            {result.outletTempC != null ? (
              <CalculatorMetricCard label="Outlet temperature" numericValue={Number(result.outletTempC.toFixed(1))} unit="°C" />
            ) : null}
