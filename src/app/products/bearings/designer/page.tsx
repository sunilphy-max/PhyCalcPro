...
function BearingDesignerPage() {
  ...
  const [uiComplexity, setUiComplexity] = useState<"simple" | "expert">("expert");
  ...
  const runCheck = () => {
    ...
    const shouldDiagnose = designerIntent === "service" || workflowMode === "diagnose";
    setDiagnosis(
      shouldDiagnose
        ? diagnoseBearing(raw, {
            radialLoad: Fr,
    ...
    publishHandoff("housing", {
      ...
      fromTitle: "Bearing System Designer",
  ...
  const calculate = () => {
    if (designerIntent === "design" && (workflowMode === "design" || workflowMode === "select")) {
  ...
  const crossManufacturerRecommendation = useMemo(() => {
    if (!result || designerIntent === "service" || workflowMode === "diagnose") return null;
    const Fr = toBase(radialLoad, "force", radialUnit) * shockFactor;
    const Fa = toBase(axialLoad, "force", axialUnit) * shockFactor;
    ...
    <CalculatorLayout
      ...
      title={
        designerIntent === "service"
          ? "Bearing System Designer — Check / Diagnose"
      ...
      inputs={
        <div className="space-y-4">
          ...
          />
          <BearingInputs
            designerIntent={designerIntent}
            designerStage={designerStage}
            onDesignerStageChange={syncStageToUrl}
