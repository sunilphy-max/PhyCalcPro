...
> = {
  ...
  springs: {
    ...
    catalogTables: ["Spring wire sizes", "Material modulus", "End condition factors"],
    linkedWorkflowModuleIds: ["fatigue", "materials/database", "safety-factor"],
    expertNotes: ["Spring design mode should reject poor spring index and solid-height conflicts."],
    gaps: ["Full wire catalog and fatigue standards need deeper integration."],
  },
  fasteners: {
    maturity: "workflow",
    modes: DEFAULT_MODES,
...
const MODULE_OVERRIDES: Record<string, Partial<ModuleDesignWorkflow>> = {
  beams: {
    ...
    gaps: ["Automatic rolled-section selection is a next step; current workflow panel shows the selection contract."],
  },
  shafts: {
    maturity: "solver-backed",
