 ...
 *
 * Classical Raimondi–Boyd: low Sommerfeld (heavy load / low speed) → high ε;
 * high Sommerfeld (light load / high speed) → low ε.
 */

/** Sommerfeld S → eccentricity ratio ε (full journal screening). */
type Curve = readonly [number, number][];

/** L/D ≈ 1 screening curve (ε falls as S rises). */
const RB_LD_1: Curve = [
  [0.001, 0.97],
  [0.01, 0.91],
  [0.02, 0.85],
...
function scaleCurve(base: Curve, towardOne: number): Curve {
  return base.map(([s, e]) => [s, Math.min(0.99, Math.max(0.01, e + (1 - e) * towardOne))] as [number, number]);
}
...
function interpolateCurve(curve: Curve, S: number): number {
  if (S <= 0) return 0.99;
  const [sFirst, eFirst] = curve[0]!;
  if (S <= sFirst) return eFirst;
  const [sLast, eLast] = curve[curve.length - 1]!;
  if (S >= sLast) return eLast;
...
/** Petroff concentric viscous friction power (W) — ISO 7902 screening. */
export function petroffPowerLossW(params: {
  viscosityPas: number;
  speedRpm: number;
  diameterM: number;
  lengthM: number;
  ...
  const r = params.diameterM / 2;
  const omega = (2 * Math.PI * params.speedRpm) / 60;
  const c = Math.max(params.radialClearanceM, 1e-12);
  return (2 * Math.PI * params.viscosityPas * omega ** 2 * r ** 3 * params.lengthM) / c;
}
